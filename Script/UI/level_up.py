...
def show_level_up_screen(screen, pokemon, menu_font, small_font, colors, clock):
    ...
    print(f"Showing level-up screen for {pokemon.name} (Level {pokemon.level})") 
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key in (pygame.K_RETURN, pygame.K_SPACE, pygame.K_z, pygame.K_x):
                    print("Level-up screen closed by user input")  
                    return

        # Auto-close after animation
        if elapsed >= animation_duration:
            print("Level-up screen auto-closed after animation")  
            return
