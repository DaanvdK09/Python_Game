...
class MultiplayerServer:
    def __init__(self):
        # Initializes the multiplayer server with socket and state management.
    ...
    def handle_client(self, client_socket, client_address):
        # Handles a new client connection, assigns ID, and processes messages.
    ...
    def handle_player_disconnect(self, client_id, battle_id):
        # Handles a player disconnecting during a battle, notifying the opponent.
    ...
    def process_message(self, client_id, message):
        # Processes incoming messages from clients and updates server state.
    ...
    def start_battle(self):
        # Starts a battle when two players are waiting.
    ...
    def handle_battle_action(self, client_id, message):
        # Handles battle actions like fight, switch, or run.
        battle_id = self.active_battles.get(client_id)
        if battle_id is not None:
            battle = self.battle_states[battle_id]
    ...
    def process_fight_action(self, client_id, battle_id, move):
        # Processes a fight action, calculates damage, and updates battle state.
        battle = self.battle_states[battle_id]
        attacker = client_id
    ...
    def run(self):
        # Runs the server, accepting new client connections.
        while True:
